...
class TestPaypalFile(common.SavepointCase):
        ...
        cls.journal = cls.env["account.journal"].create(
            {
                ...
                "currency_id": (
                    usd.id
                    if cls.env.user.company_id.currency_id != usd
                    else False
                ),
            }
        )
    ...
    def test_import_header(self):
        ...
        wizard = (
            self.env["wizard.paypal.map.create"]
            .with_context(
                {"journal_id": self.journal.id, "active_ids": [self.map.id],}
            )
    ...
    def test_import_paypal_file(self):
        ...
        self.assertEqual(
            len(statement.mapped("line_ids").filtered(lambda x: x.partner_id)),
            1,
        )
