...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _check_paypal(self, data_file):
        ...
        if any(item not in file_headers for item in headers):
            raise UserError(
                _(
                    "Headers of file to import and Paypal map lines does not "
                    "match."
                )
            )
        return True
    ...
    def _convert_paypal_line_to_dict(self, idx, line):
        ...
        for item in range(len(line)):
            ...
            if paypal_map.date_format:
                try:
                    ...
                    raise UserError(
                        _(
                            "Date format of map file and Paypal date does "
                            "not match."
                        )
                    )
        ...
        for field in ["commission", "amount", "balance"]:
            ...
            try:
                ...
                raise UserError(
                    _(
                        "Value '%s' for the field '%s' on line %d, "
    ...
    def _prepare_paypal_statement_line(self, fline):
        ...
        return {
            ...
            "unique_import_id": fline["transaction_id"]
            + fline["date"]
            + fline["time"],
            "amount": fline["amount"],
    ...
    @api.model
    def _get_paypal_partner(
        self, description, partner_name, partner_email, invoice_number
    ):
        if invoice_number:
        ...
        if partner_email:
            partner = self.env["res.partner"].search(
                [("email", "=", partner_email), ("parent_id", "=", False)]
            )
